    ...
    def step(
        ...
        for idx, agent_id in enumerate(self._agent_ids):
            ...
            info_dict[agent_id].update(
                ...
                raw_reward_components={
                    "progress": progress_reward, "approach": approach_reward,
                    "contribution": contribution_reward, "connectivity": conn_penalty,
                    "contact": coop_contact, "energy": -energy_penalty,
                    "time": -self.step_cost if self.reward_version == "transport_v2" else 0.0,
                    "completion": 0.0,
                },
            )
        ...
        if terminated_dict["__all__"]:
            for agent_id in self._agent_ids:
                ...
                info_dict[agent_id]["raw_reward_components"]["completion"] = self.completion_bonus
            self._reward_totals["completion"] += self.completion_bonus * self.num_robots

        # Scale only the learning signal, after every term including terminal reward.
        for agent_id, raw_reward in reward_dict.items():
            reward_dict[agent_id] = raw_reward * self.reward_scale
            info_dict[agent_id].update(
                raw_reward=raw_reward, learning_reward=reward_dict[agent_id],
                reward_scale=self.reward_scale,
                scaled_reward_components={key: value * self.reward_scale for key, value in
                                          info_dict[agent_id]["raw_reward_components"].items()},
            )
